...
from contextualized import BERT, get_triple_vectors
...
class Corpus:
    def __init__(self, train_data, validation_data, test_data, entity2id,
                 relation2id, headTailSelector, batch_size, valid_to_invalid_samples_ratio, unique_entities_train, word_embedding_model, word_embedding_dim, word_embedding_path, get_2hop=False):

        if word_embedding_model is not None:
            self.tokenizer = AutoTokenizer.from_pretrained(word_embedding_model)
            self.bert = BERT(AutoModel.from_pretrained(word_embedding_model), word_embedding_dim, len(relation2id))
            self.bert.load_state_dict(torch.load(os.path.join(word_embedding_path, 'Fine_Tuned_BertModel')))
            self.bert.to(device)
            self.bert.eval()
    ...
    def set_ctx_emb(self, ctx_emb):
        self.ctx_emb = ctx_emb

    def set_ctx_nhop_emb(self, ctx_nhop_emb):
        self.ctx_nhop_emb = ctx_nhop_emb

    def get_contextualized_embeddings(self, args, edges):
        edge_list = edges[0]
        edge_type = edges[1]
        #print(edge_list.shape, edge_type.shape)

        ctx_emb = []
        for i in tqdm(range(edge_type.shape[0])): 
            #print(edge_list[0, i], edge_type[i], edge_list[1, i])
            r = self.id2relation[edge_type[i].item()]
            t = self.id2entity[edge_list[0, i].item()]
            h = self.id2entity[edge_list[1, i].item()]
            H, R, T, C = get_triple_vectors(args, h, r, t, self.tokenizer, self.bert)
            ctx_emb.append(C.cpu())   
        return torch.stack(ctx_emb)

    def get_nhop_contextualized_embedding(self, args, edge_list, edge_type):
        print(edge_list[:, 0], edge_type[0, :], edge_type.shape[0])
        ctx_emb = []
        out = {}
        for i in tqdm(range(edge_type.shape[0])):
            H, T, R = [None, None], [None, None], [None, None]
            for j in range(2):
                # print(edge_list[0, i], edge_type[i, j], edge_list[1, i])                
                r = self.id2relation[edge_type[i, j].item()]
                t = self.id2entity[edge_list[0, i].item()]
                h = self.id2entity[edge_list[1, i].item()]
                H[j], R[j], T[j], C = get_triple_vectors(args, h, r, t, self.tokenizer, self.bert)
            R = R[0] + R[1]
            H = H[0] + H[1]
            T = T[0] + T[1]
            C = torch.cat((R, H, T))
            out[(int(edge_list[0, i].item()), int(edge_list[1, i].item()))] = C
            ctx_emb.append(C)
        return out #torch.stack(ctx_emb)
    ...
    def get_validation_pred(self, args, model, unique_entities):
        ...
        for iters in range(1):
            ...
            for i in range(batch_indices.shape[0]):
                ...
                top_ten_head = [(self.id2entity[new_x_batch_head[j][0]], self.id2relation[new_x_batch_head[j][1]], self.id2entity[new_x_batch_head[j][2]], sorted_scores_head.cpu().numpy()[i], j.cpu().numpy() == 0) for i, j in enumerate(sorted_indices_head[:10])]
                top_ten_tail = [(self.id2entity[new_x_batch_tail[j][0]], self.id2relation[new_x_batch_tail[j][1]], self.id2entity[new_x_batch_tail[j][2]], sorted_scores_tail.cpu().numpy()[i], j.cpu().numpy() == 0) for i, j in enumerate(sorted_indices_tail[:10])]
                tops.extend(top_ten_head)
                tops.extend(top_ten_tail)
            df = pd.DataFrame(tops, columns=['head', 'relation', 'tail', 'score', 'in_test'])
            df.to_csv(os.path.join(args.results_path, 'predictions.csv'), mode='a', header=not os.path.exists(os.path.join(args.results_path, 'predictions.csv')))
        ...
        epochs = str(args.epochs_gat) + '_' + str(args.epochs_conv)
        model = 'Baseline' + ('' if not args.use_ctx else '_' + args.word_embedding_model) + '_' + args.embedding_model
        row = [args.dataset, model, epochs, datetime.now().strftime("%d/%m/%Y %H:%M:%S"), 
        ...
        df.to_csv(os.path.join(args.results_path, 'results.csv'), mode='a', header=not os.path.exists(os.path.join(args.results_path, 'results.csv')))
