...
class SpGraphAttentionLayer(nn.Module):
    ...
    def __init__(self, num_nodes, in_features, out_features, nrela_dim, dropout, alpha, ctx_dim, concat=True):
        ...
        self.a = nn.Parameter(torch.zeros(
            size=(out_features, 2 * in_features + nrela_dim + 3 * ctx_dim)))
    ...
    def forward(self, input, edge, edge_embed, edge_list_nhop, edge_embed_nhop, ctx_emb):
        N = input.size()[0]
        ...
        if self.ctx_dim > 0:
            edge_h = torch.cat(
                (input[edge[0, :], :], input[edge[1, :], :], edge_embed[:, :], ctx_emb[:, :]), dim=1).t()
        else:
            edge_h = torch.cat(
